...
void setup() {
  ...
  for (int i = 0; i<a.length; i++) {
    a[i] = new Asteroid(int(random(width)), int(random(-10000, 100)), 20, 20, int(random(1, 4)));
  }
}
...
void display() {
  ...
  text("Shots Fired: " + int(fired), width/9, height-10);
  text("Targets Hit: " + int(hit), width/5, height-30);
