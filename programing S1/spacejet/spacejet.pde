...
void setup() {
  ...
  for (int i = 0; i<a.length; i++) {
    a[i] = new Asteroid(int(random(width)), int(random(-15000, 100)), 20, 20, int(random(1, 4)));
  }
}
...
void mousePressed() {
  ...
  l1.shoot();
  for (int i=0; i<a.length; i++) {
    if (a[i].destroy(l1.lx1, l1.ly1, l1.h, l1.h, l1.laser)) { 
      if (a[i].bigRock) {
        updateScores(true);
      } else {
        updateScores(false);
...
void display() {
  ...
  text("Missed: " + missed, 10, height-5);
  text("Shots Fired: " + int(fired), width/1.5, height-7);
  text("Targets Hit: " + int(hit), width/2, height-30);
  text("Efficiency: " + ((hit)/(fired))*100 + "%", width/4.3, height-7);
  text("Level: " + s1.pu, width/4, height-30);
}
...
void gameOver() {
  ...
  textSize(30);
  fill(222);
  text("Final Score: " + score, 45, 390);
  text("Points: " + score, 45, 430);
  text("Missed: " + missed, 45, 470);
  text("Shots Fired: " + int(fired), 45, 510);
  text("Objects Hit: " + int(hit), 45, 550);
  text("Efficiency: " + ((hit)/(fired))*100 + "%", 45, 590);
  noLoop();
}